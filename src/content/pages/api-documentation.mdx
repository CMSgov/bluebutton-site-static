...
description: "Get comprehensive API documentation for CMS Blue Button, including instructions on how to access sandbox and production claims data."
---
...
Medicare enrollees may always opt to omit personal data such as name, date of birth, race, and sex when granting access to claims data. An enrollee's choice during the Blue Button authorization flow determines the [scope](/api-docs/authorization#scopes) of access an application will have to their data. 
...
We currently support 2 Blue Button API versions:
...
Version 1 (V1) is the original Blue Button API, based on [FHIR release 3 (STU3)](http://hl7.org/fhir/STU3/). Development on V1 is limited to bug fixes and basic maintenance. If you're developing a new app, use V2\. For more information about Blue Button V1, [contact us](/support). 

## Environments

We currently maintain production and sandbox environments for developing with the Blue Button API. For technical information about our environments, see [Base FHIR URLs](/api-docs/calling-the-api#base-fhir-urls). 
