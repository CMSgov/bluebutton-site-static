...
Parameters in `POST` request to Blue Button's `/token` endpoint for CMS-aligned networks flow:
...
Blue Button will perform patient matching based upon the presence of an IAL2 `id_token` provided by a registered client application. Blue Button will seek specific fields in the `id_token`'s claims.
...
Approved field combinations that Blue Button currently supports:
