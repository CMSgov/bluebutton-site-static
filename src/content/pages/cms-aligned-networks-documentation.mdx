...
import networkFlow from '#assets/images/cms-aligned-network-flow.png';
...
<Image class="width-full border-1px border-base-lighter padding-2" src={networkFlow} alt="Sequence diagram showing the CMS-Aligned Network authentication flow between Patient, Consumer App, CSP, Blue Button, and BFD, including AAL2 authentication, IAL2 identity verification, token exchange, patient matching, and FHIR data retrieval" />
...
| `scope` | required | Any values from Blue Button API's `./well-known/smart-configuration` are valid. <br/><br/>**Example:** <ul><li>`patient/Patient.rs`</li><li>`patient/Coverage.rs`</li><li>`patient/ExplanationOfBenefit.rs`</li> <li>`launch/patient`</li> <li>`profile`</li></ul> |
...
```json
{
  ...
  "scope": "patient/Patient.rs patient/Coverage.rs patient/ExplanationOfBenefit.rs launch/patient profile",
...
<Alert size="slim" noIcon>**Note:** `jku` will be ignored if provided</Alert>
...
<Alert size="slim" noIcon>**Note:** These may require updates depending upon any late-breaking alignment from CSPs and workgroups.</Alert>
...
<Alert size="slim" noIcon>**Note:** Contact your Credential Service Provider in order to confirm `identity_assurance_level` and `ssn_itin_short` are present on your application</Alert>

| Element | Optionality | Requirement |
| :---- | :---- | :---- |
| `iss` | required | <ul><li>**Production**: Must be `https://api.id.me/oidc` or `https://verified.clearme.com/integrations`</li><li>**Lower environments**: Must be `https://api.idmelabs.com/oidc` or `https://verified.clearme.com/integrations`</li></ul>  |
...
| `identity_assurance_level` | **required** | **Must > = 2 in CSP-signed payload** |
