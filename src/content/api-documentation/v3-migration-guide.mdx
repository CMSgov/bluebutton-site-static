...
* Addition of Structure Definition and CodeSystem resources
...
* `ExplanationOfBenefit.meta.lastUpdated` continues to represent the time the EOB was last updated in BFD. 
...
v3 resources conform to CARIN for Blue Button version 2.1.0, which itself builds upon US Core 6.1.0. BFD v3 supports FHIR R4.
...
To identify duplicates using the claim control number, you can identify all claims for a given enrollee with the same `ExplanationOfBenefit.billablePeriod.start` that share the same `CLM-CNTL-NUM` (in either identifier or related). BFD currently provides the latest version of the claim.
...
<Alert size="slim" variant="info">**Note:** Phase 1 (claim type code 1XXX) claims can contain many duplicates that may not be marked as related upstream of BFD. This means BFD cannot deduplicate them based on the claim that eventually gets processed. These claims are abandoned in claims processing.</Alert>
