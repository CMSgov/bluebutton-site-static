...
title: 'Authorization'
seo: 
  description: "Learn how to implement the OAuth 2.0 authorization flow for the Blue Button API, including PKCE, token management, and scope permissions for Medicare data access"
sortOrder: 3
---

The Blue Button API uses the [OAuth 2 authorization flow](https://www.oauth.com/). To initiate an integration, you'll need the client ID and client secret for your application that were generated when you registered your application.
...
- To explicitly specify the language for the Blue Button authorization screens, add the optional `lang` parameter to your `/authorize` request. Valid values are `en` (English) and `es` (Spanish). For example: 
...
Note that for applications in the "1 hour" or "13 months" access expiration categories, the response includes the `access_grant_expiration` date for that user.
...
If you receive this message, double-check that the request looks correct. If everything looks correct, email [bluebuttonapi@cms.hhs.gov](mailto:bluebuttonapi@cms.hhs.gov), and the Blue Button API team can help troubleshoot.
...
Scopes define the API endpoints that your application is allowed to access. The Blue Button API uses [HL7 FHIR Scopes](https://hl7.org/fhir/smart-app-launch/scopes-and-launch-context.html) to manage access to Medicare enrollee data. 
