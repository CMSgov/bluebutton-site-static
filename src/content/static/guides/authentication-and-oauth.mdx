...
```http
https://sandbox.bluebutton.cms.gov/v3/o/authorize/?lang=es
```
...
```bash title="Terminal"
curl -X POST --url 'https://bluebutton.cms.gov/v3/o/revoke/'\
...
```bash title="Terminal"
curl -X POST "https://sandbox.bluebutton.cms.gov/v3/o/expire_authenticated_user/-20140000000001/" \
  -u "<client_id>:<client_secret>" \
  -H "Content-Length: 0"
```
...
The Blue Button API supports the [OAuth 2.0 Authorization Framework](https://www.rfc-editor.org/info/rfc6749/) using the [authorization code grant](https://www.rfc-editor.org/rfc/rfc6749#section-1.3.1) with a [confidential client](https://www.rfc-editor.org/rfc/rfc6749#section-2.1) type flow. To keep Medicare enrollees’ data secure during authentication, we don’t support implicit grants or public client types.
...
* Follows the [OAuth 2.0 Security Best Current Practice](https://www.rfc-editor.org/info/rfc9700/#section-2)
