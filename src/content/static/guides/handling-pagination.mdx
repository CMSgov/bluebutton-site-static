...
```json
{
  ...
  "link": [
    ...
    {
      "relation": "next",
      "url": "https://sandbox.bluebutton.cms.gov/v3/fhir/ExplanationOfBenefit?startIndex=10&_count=10&patient=..."
    },
  ],
  "entry": [ ... ]
}
...
<Alert>**Note:** `_lastUpdated` returns added and modified claims only: it can't tell you a claim was removed, so schedule a periodic full resync. On v3, pass `_source` or `_tag` alongside it; otherwise Blue Button applies `_source=NCH` and your sync will miss every update to partially adjudicated claims.</Alert>

<Alert size="slim" variant="success">**Tip:** For most apps, the simplest approach is: do a full fetch on first load bounded with `le{timestamp}`, then use `_lastUpdated` for incremental updates going forward, with a periodic full resync to catch removals.</Alert>
