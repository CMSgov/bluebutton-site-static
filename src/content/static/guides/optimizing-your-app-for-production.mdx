...
Use `service-date` for the care period, and `_lastUpdated` for what's changed since your last sync. They're independent and can be combined.
...
The Blue Button API aligns with the [CARIN Consumer Directed Payer Data Exchange Implementation Guide](https://hl7.org/fhir/us/carin-bb/). If your app works with other payer APIs, following the CARIN IG ensures interoperability.
...
If you're building for Blue Button only, you don't need to study the full IG, but it helps to be aware of it if you plan to integrate with other FHIR APIs.

Reference:

- [CARIN IG](https://hl7.org/fhir/us/carin-bb/) 

## Testing checklist

Before your production demo, test these scenarios:

* **Multiple users** Test with several [synthetic users](data/#synthetic-data) to find users with different claim volumes). 
* **Large datasets** Some synthetic users have hundreds of claims. Verify your [pagination handles them](/guides/handling-pagination/).
* **Token expiration** Use the sandbox `/expireauthenticateduser` endpoint (see [Testing token expiration](/guides/authentication-and-oauth/#testing-token-expiration)) to test expired tokens.
* **Revoked access** Test what happens when a user disconnects your app.
* **All claim types** Verify your app handles [carrier, inpatient, outpatient, PDE, DME, HHA, hospice, and SNF claims](/guides/working-with-claims/#claim-types).

## Next steps 

Ready to apply? Head to the [Production access overview](/production-access/) to see the application process, requirements checklist, and what to expect in the demo.
