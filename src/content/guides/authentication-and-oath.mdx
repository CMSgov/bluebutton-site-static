...
2. The user logs into [Medicare.gov](http://Medicare.gov) with [Login.gov](http://Login.gov), CLEAR, [ID.me](http://ID.me), or their [Medicare.gov](http://Medicare.gov) account and grants your app access  
...
To learn more about setting up your sandbox application, see [Quickstart](src/content/quickstart/Quickstart.mdx).
...
The user logs in at Medicare.gov and sees what data your app is requesting. If they click Connect, Blue Button redirects back to your redirect\_uri with a code and state parameter. For example:
...
If the user authorizes your application, the Blue Button API redirects them back to the redirect\_uri you registered. It appends an authorization code and the state parameter sent by your application to the URL.
...
curl -X POST "https://sandbox.bluebutton.cms.gov/v3/o/token/"\
 ...
 -d "code=TSjqiZCdJwGyytGjz2GzziPfHTJ6z2&grant_type=authorization_code&redirect_uri=http%3A%2F%2Flocalhost%3A8080%2Ftestclient%2Fcallback
...
For applications in the “1 hour” or “13 months” access expiration groups, the response shows the user's access\_grant\_expiration date.

See which [access category](guides/fetching-patient-data.mdx#duration-of-access) best fits your app.
...
For testing in our sandbox, use the `/expire\_authenticated\_user` endpoint. This will expire the authorization given by a patient user.

`/expire\_authenticated\_user` can be used to test your code for the following conditions that produce the same error responses via the API:
...
POST to the Blue Button API `/expire\_authenticated\_user` endpoint with the following parameters:
...
1. Generate a random code\_verifier (43-128 characters, URL-safe)  
2. Compute the challenge: code\_challenge \= BASE64URL(SHA256(code\_verifier))  
3. Send code\_challenge and code\_challenge\_method=S256 in the /authorize request  
4. Send the original code\_verifier when exchanging the code for a token
## Scopes 
...
To re-authorize, follow the same flow described under [Get an authorization code](#guides/authentication-and-oath.mdx#get-an-authorization-code-existing-updated-content).
